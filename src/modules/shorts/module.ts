...
import { SHORTS_PACING } from './pacing';
...
import { makeGenerateStoryboard } from './storyboard-provider';
import { makeSuggestBrief } from './suggest-brief';
import { registerShortsCommands } from './cli';
...
/**
 * The short-form pipeline, minus the subject matter.
 *
 * A sentence about something becomes a thirty-to-sixty second 9:16 video with
 * Vietnamese narration, shot against photographs the engine finds itself. Every
 * decision in here - the Shorts ceiling, the crop tolerance, the Vietnamese
 * voice check - follows from *the format*, and the format is the same whether
 * the channel is telling you about octopus hearts or about why your brain does
 * that thing in lifts.
 *
 * What is left over is the writing, and that arrives as `prompts`. See
 * `prompts.ts` for why that is the whole seam.
 */
export interface ShortsChannel {
  id: ModuleId;
  /** Shown in the UI and in CLI help. */
  label: string;
  /** One line describing what this channel makes. */
  description: string;
  /** How this channel asks for a script. The only thing that differs. */
  prompts: ShortsPrompts;
}

export function makeShortsModule(channel: ShortsChannel): VideoModule<Brief, Scene, Storyboard> {
  return {
  id: channel.id,
  label: channel.label,
  description: channel.description,

  pacing: SHORTS_PACING,
  /**
  ...
  generateStoryboard: makeGenerateStoryboard(channel.prompts),
  suggestBrief: makeSuggestBrief(channel.prompts),
  registerCommands: registerShortsCommands,
  };
}
